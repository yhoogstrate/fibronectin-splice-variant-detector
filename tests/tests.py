...
class Tests(unittest.TestCase):
    def test_001(self):
        ...
        self.assertEqual(fn1splicevardeterminer.extract_FN1_7B89_sv_reads(input_file_bam, fn1splicevardeterminer.egfr_exons[dbkey], False, False, EXONS_WT_ALL, EXONS_VIII_ALL), {'vIII': {'example_01'}, 'wt': set()})
        self.assertEqual(fn1splicevardeterminer.extract_FN1_7B89_sv_reads_based_on_sjs(input_file_bam, fn1splicevardeterminer.egfr_exons[dbkey], False, False), {'vIII': {'example_01'}, 'wt': set()})
    ...
    def test_002(self):
        ...
        self.assertEqual(fn1splicevardeterminer.extract_FN1_7B89_sv_reads(input_file_bam, fn1splicevardeterminer.egfr_exons[dbkey], False, False, EXONS_WT_ALL, EXONS_VIII_ALL), {'vIII': set(), 'wt': {'example_002'}})
        
        # spliced only - hier moet die hem niet vinden, in wt
        self.assertEqual(fn1splicevardeterminer.extract_FN1_7B89_sv_reads_based_on_sjs(input_file_bam, fn1splicevardeterminer.egfr_exons[dbkey], False, False), {'vIII': set(), 'wt': {'example_002'}})
    ...
    def test_003(self):
        ...
        # hier moet die hem wel vinden, in wt
        results = fn1splicevardeterminer.extract_FN1_7B89_sv_reads(input_file_bam, fn1splicevardeterminer.egfr_exons[dbkey], False, True, EXONS_WT_ALL, EXONS_VIII_ALL)
        self.assertEqual(len(results['vIII']), 170)
        self.assertEqual(len(results['wt']), 0)
        
        # do not allow PCR/optical duplicates
        results = fn1splicevardeterminer.extract_FN1_7B89_sv_reads(input_file_bam, fn1splicevardeterminer.egfr_exons[dbkey], False, False, EXONS_WT_ALL, EXONS_VIII_ALL)
        self.assertEqual(len(results['vIII']), 43)
        self.assertEqual(len(results['wt']), 0)
        
        # spliced only - hier moet die hem niet vinden, in 
        results = fn1splicevardeterminer.extract_FN1_7B89_sv_reads_based_on_sjs(input_file_bam, fn1splicevardeterminer.egfr_exons[dbkey], False, True)
    ...
    def test_004(self):
        ...
        # spliced only - hier moet die hem niet vinden, in 
        results = fn1splicevardeterminer.extract_FN1_7B89_sv_reads_based_on_sjs(input_file_bam, fn1splicevardeterminer.egfr_exons[dbkey], False, False)
        self.assertEqual(len(results['vIII']), 0)
        self.assertEqual(len(results['wt']), 0)

        # ook niet spliced, hier moet je hem wel in vinden
        results = fn1splicevardeterminer.extract_FN1_7B89_sv_reads_based_on_sjs(input_file_bam, fn1splicevardeterminer.egfr_exons[dbkey], True, False)
